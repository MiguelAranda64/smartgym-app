...
const LoginScreen = () => {
  ...
  async function submit() {
    try {
      const res = await axios.post("http://192.168.0.15:9000/", {
        username,
        password,
      });
  
      if (res.data === "exist") {
        navigation.navigate('Rutinas',{ id: username }); //AQUI VA LA REDIRECCION DE LA PAGINA DESPUES DE LOGIN
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('AdminScreen')}>
        <Text style={styles.adminLink}>¿Eres administrador? Entra aquí</Text>
      </TouchableOpacity>
    </View>
