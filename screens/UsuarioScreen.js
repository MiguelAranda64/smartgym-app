...
const UsuarioScreen = () => {
  return (
    <Drawer.Navigator drawerContent={CustomDrawerContent}>
      <Drawer.Screen name="Usurio" component={UsuarioScreenContent} />
    </Drawer.Navigator>
  );
}
...
const UsuarioTable = () => {
  ...
  const fetchUsuarioData = async () => {
    try {
      const responseUsuario= await fetch(
        "http://192.168.0.13:8888/api/usuarios"
      );
  ...
  const insertUsuario = async () => {
    try {
      ...
      const response = await fetch("http://192.168.0.13:8888/api/usuarios", {
        method: "POST",
        headers: {
  ...
  const deleteUsuario = async (item) => {
    try {
      const response = await fetch(
        `http://192.168.0.13:8888/api/usuarios/${item._id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const updateUsuario = async () => {
    try {
      const response = await fetch(
        `http://192.168.0.13:8888/api/usuarios/${form._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
